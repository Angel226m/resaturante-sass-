...
import {
  ...
  CalendarDays, UtensilsCrossed, Receipt, BellRing, Eye,
} from 'lucide-react';
...
// 1. Seleccionar mesa -> 2. Armar pedido del menu -> 3. Enviar a cocina
...
const estadoMesa: Record<string, { color: string; bg: string; label: string; icon: React.ReactNode; ring: string }> = {
  disponible: { color: 'text-emerald-600', bg: 'bg-gradient-to-br from-emerald-50 to-green-50 border-emerald-200', label: 'Libre', icon: <Sparkles className="h-5 w-5" />, ring: 'ring-emerald-400' },
  ocupada: { color: 'text-blue-600', bg: 'bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200', label: 'Ocupada', icon: <Users className="h-5 w-5" />, ring: 'ring-blue-400' },
  reservada: { color: 'text-amber-600', bg: 'bg-gradient-to-br from-amber-50 to-yellow-50 border-amber-200', label: 'Reservada', icon: <Coffee className="h-5 w-5" />, ring: 'ring-amber-400' },
  fuera_servicio: { color: 'text-red-600', bg: 'bg-gradient-to-br from-red-50 to-rose-50 border-red-200', label: 'No disponible', icon: <Ban className="h-5 w-5" />, ring: 'ring-red-400' },
};
...
export default function MeseroDashboard() {
  ...
  const { data: ordenes = [] } = useQuery({
    queryKey: ['ordenes', ordenesParams],
    queryFn: () => ordenesRepository.listarOrdenes(ordenesParams),
    enabled: !!usuario?.id,
  ...
  const storageVistosKey = `mesero:vistos:listos:${usuario?.tenant_id || 'na'}:${localId || usuario?.local_id || 'na'}:${usuario?.id || 'na'}`;
  ...
  const pedidosListosPendientes = useMemo(
    () => misOrdenes.filter((o: Orden) => o.estado === 'lista' && !ordenesVistas.includes(Number(o.id))),
    [misOrdenes, ordenesVistas],
  );

  useEffect(() => {
    try {
      const raw = localStorage.getItem(storageVistosKey);
      if (!raw) {
        setOrdenesVistas([]);
        return;
      }
      ...
      if (Array.isArray(parsed)) {
        setOrdenesVistas(parsed.map((v) => Number(v)).filter((v) => Number.isFinite(v)));
      } else {
        setOrdenesVistas([]);
      }
    } catch {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(storageVistosKey, JSON.stringify(ordenesVistas));
    } catch {
      // Ignorar errores de storage.
    }
  }, [storageVistosKey, ordenesVistas]);

  useEffect(() => {
    if (misOrdenes.length === 0) return;
    const idsActuales = new Set(misOrdenes.map((o) => Number(o.id)));
    setOrdenesVistas((prev) => {
      const depurados = prev.filter((id) => idsActuales.has(id));
      return depurados.length === prev.length ? prev : depurados;
    });
  }, [misOrdenes]);
  ...
  const handleEnviarACocina = () => {
    ...
    if (!localId) { toast.error('Selecciona un local antes de enviar'); return; }
  ...
  const greeting = () => { const h = new Date().getHours(); return h < 12 ? 'Buenos dias' : h < 18 ? 'Buenas tardes' : 'Buenas noches'; };

  const marcarPedidoVisto = (ordenId: number) => {
    setOrdenesVistas((prev) => (prev.includes(ordenId) ? prev : [...prev, ordenId]));
  };

  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  // STEP 2: MenÃº + Carrito â€” tomar pedido
  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  if (step === 'menu' && mesaSeleccionada) {
    return (
      <div className="flex h-[calc(100vh-4rem)] gap-0 -m-6">
        {/* â–¸â–¸â–¸ Panel izquierdo: MenÃº */}
        <div className="flex-1 flex flex-col overflow-hidden bg-slate-50">
          {/* Header menÃº */}
          <div className="flex items-center gap-4 px-6 py-4 bg-white border-b border-slate-200">
            <button onClick={resetOrder} className="flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700 transition-colors">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-bold text-slate-900">Mesa {mesaSeleccionada.numero}</p>
                <p className="text-[10px] text-slate-500">{numPersonas} personas</p>
              </div>
            </div>
            <div className="flex items-center gap-2 ml-2">
              <button onClick={() => setNumPersonas(Math.max(1, numPersonas - 1))} className="h-6 w-6 rounded-md border border-slate-300 flex items-center justify-center hover:bg-slate-100 transition-colors">
                <Minus className="h-3 w-3" />
              </button>
              <span className="text-xs font-medium w-4 text-center">{numPersonas}</span>
              <button onClick={() => setNumPersonas(numPersonas + 1)} className="h-6 w-6 rounded-md border border-slate-300 flex items-center justify-center hover:bg-slate-100 transition-colors">
            ...
            <div className="relative w-64">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
              />
            </div>
          </div>

          {/* CategorÃ­as */}
          <div className="flex gap-2 px-6 py-3 overflow-x-auto border-b border-slate-100 bg-white scrollbar-hide">
            <button
              onClick={() => setCatActiva(null)}
              className={`shrink-0 rounded-full px-4 py-1.5 text-xs font-semibold transition-all ${!catActiva ? 'bg-teal-500 text-white shadow-md shadow-teal-500/25' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
            >
              Todos
            </button>
            {categorias.map((c: CategoriaMenu) => (
              <button
                ...
                className={`shrink-0 rounded-full px-4 py-1.5 text-xs font-semibold transition-all ${catActiva === c.id ? 'bg-teal-500 text-white shadow-md shadow-teal-500/25' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
              >
                ...
                {productosFiltrados.map((p: ProductoMenu) => {
                  ...
                  return (
                    <button
                      key={p.id}
                      onClick={() => addToCart(p)}
                      className={`group relative rounded-2xl border-2 bg-white p-4 text-left transition-all hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 ${qty > 0 ? 'border-teal-400 shadow-md shadow-teal-500/10 ring-1 ring-teal-400/30' : 'border-slate-200'}`}
                    >
                      {/* Qty badge */}
                      {qty > 0 && (
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-2xl">{cat?.icono || 'PL'}</span>
                        {(p as any).es_especialidad && <Badge variant="teal" className="text-[9px]">Especial</Badge>}
                      </div>
                      <h4 className="text-sm font-bold text-slate-900 leading-tight">{p.nombre}</h4>
                      ...
                      <div className="flex items-center justify-between mt-3">
                        ...
                        <span className="flex h-7 w-7 items-center justify-center rounded-full bg-teal-50 text-teal-600 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        {/* â–¸â–¸â–¸ Panel derecho: Carrito */}
        <div className="w-[380px] shrink-0 flex flex-col bg-white border-l border-slate-200">
          {/* Cart header */}
          ...
          <div className="flex-1 overflow-y-auto">
            {cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full px-6 text-center">
                ...
                <p className="text-sm font-medium text-slate-500">Carrito vacio</p>
                <p className="text-xs text-slate-400 mt-1">Selecciona platos del menu</p>
              </div>
            ) : (
              <div className="p-4 space-y-2">
                {cart.map(item => (
                  <div key={item.producto.id} className="rounded-xl border border-slate-100 p-3 hover:bg-slate-50/50 transition-colors">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <div className="flex items-center gap-1">
                        <button onClick={(e) => { e.stopPropagation(); removeFromCart(item.producto.id); }} className="h-7 w-7 rounded-lg border border-slate-200 flex items-center justify-center hover:bg-red-50 hover:border-red-300 hover:text-red-500 transition-colors">
                          <Minus className="h-3 w-3" />
                        </button>
                        <span className="w-8 text-center text-sm font-bold text-slate-900">{item.cantidad}</span>
                        <button onClick={(e) => { e.stopPropagation(); addToCart(item.producto); }} className="h-7 w-7 rounded-lg border border-slate-200 flex items-center justify-center hover:bg-teal-50 hover:border-teal-300 hover:text-teal-500 transition-colors">
                          <Plus className="h-3 w-3" />
                        </button>
                      </div>
                      <div className="flex items-center gap-1">
                    ...
                    {showNotasItem === item.producto.id && (
                      <div className="mt-2">
                        <input
                          ...
                          className="w-full rounded-lg border border-slate-200 px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-amber-400"
                          autoFocus
                        />
                      </div>
                    )}
          ...
          {cart.length > 0 && (
            <div className="border-t border-slate-200 p-5 space-y-3 bg-slate-50/50">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-200 px-4 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-teal-400"
              />
              {/* Totals */}
  ...
  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  // STEP 1: Dashboard â€” ver mesas + Ã³rdenes activas
  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard title="Mis ordenes activas" value={misOrdenes.length} icon={<ShoppingCart className="h-5 w-5" />} color="blue" />
                ...
                {mesas.map((mesa: Mesa) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-2xl font-black text-slate-900">{mesa.numero}</p>
            ...
            <Card>
              ...
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 px-5 pb-5">
                {reservasHoy.slice(0, 6).map((r: any) => (
                  <div key={r.id} className="rounded-xl border border-slate-200 p-3 hover:shadow-sm transition-shadow">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-semibold text-slate-900">{r.nombre_contacto}</p>
        ...
        <div>
          <Card>
            <div className="p-5 pb-3 border-b border-slate-100">
              <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
                <BellRing className="h-5 w-5 text-emerald-500" /> Pedidos listos por entregar
              </h2>
              <p className="text-xs text-slate-500 mt-1">Se mantienen aqui hasta que los marques como vistos.</p>
            </div>
            {pedidosListosPendientes.length === 0 ? (
              <div className="p-5">
                <p className="text-sm text-slate-500">Sin pedidos listos pendientes.</p>
              </div>
            ) : (
              <div className="space-y-2 p-4">
                {pedidosListosPendientes.map((orden: Orden) => (
                  <div key={`listo-${orden.id}`} className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-sm font-bold text-emerald-800">Mesa {orden.mesa_id || '-'}</p>
                        <p className="text-xs text-emerald-700">Pedido #{orden.numero_orden}</p>
                      </div>
                      <button
                        onClick={() => marcarPedidoVisto(Number(orden.id))}
                        className="inline-flex items-center gap-1 rounded-lg bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 border border-slate-200 hover:bg-slate-50"
                      >
          ...
          <Card>
            <div className="p-5 pb-3">
              <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                <ShoppingCart className="h-5 w-5 text-indigo-500" /> Mis Ordenes
              </h2>
            </div>
            {misOrdenes.length === 0 ? (
              <div className="p-5 pt-0"><EmptyState icon={<ShoppingCart className="h-10 w-10" />} title="Sin ordenes" description="Selecciona una mesa para crear un pedido" /></div>
            ) : (
                ...
                {misOrdenes.map((orden: Orden) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full flex items-center justify-between rounded-xl border border-slate-200 p-3 hover:bg-slate-50 transition-colors text-left"
                    >
                      <div>
                        <p className="text-sm font-bold text-slate-900">#{orden.numero_orden}</p>
                        <p className="text-xs text-slate-500">Mesa {orden.mesa_id} - {(orden.items || []).length} platos</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-bold text-slate-900">{formatCurrency(orden.total)}</p>
            ...
            <div className="space-y-2">
              {(showOrdenDetalle.items || []).map((item, i) => (
                <div key={i} className="flex items-center justify-between rounded-lg bg-slate-50 px-4 py-2">
                  <div>
                    <span className="text-sm font-semibold text-slate-900">{item.cantidad}x</span>
                    <span className="text-sm text-slate-700 ml-2">{item.nombre_producto}</span>
